...
#
#!\file run_ut.py
#!\brief Unit test runner for the komply command-line tool.
...
UT_ROOT = REPO_ROOT / "src" / "tst" / "ut"

if str(LIB_ROOT) not in sys.path:
    sys.path.insert(0, str(LIB_ROOT))
if str(UT_ROOT) not in sys.path:
    sys.path.insert(0, str(UT_ROOT))
...
def main() -> int:
    ...
    suite = loader.discover(start_dir=str(UT_ROOT), pattern="test_*.py")
    total = suite.countTestCases()
    print(f"{LABEL_HEADER} Running {total} unit test(s)")
